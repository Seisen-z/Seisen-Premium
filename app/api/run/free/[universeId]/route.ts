...
export async function GET(
  ...
  let freeUrl = 'https://raw.githubusercontent.com/Mentos4/roblox/refs/heads/main/gamelist.lua';
  try {
    const { data: config } = await supabase.from('github_config').select('free_url').single();
    if (config?.free_url) freeUrl = config.free_url;
  } catch {}

  const urls = await getGamelistUrls(freeUrl);
