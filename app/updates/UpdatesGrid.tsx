...
export default function UpdatesGrid({ updates }: { updates: Update[] }) {
  ...
  // Build tag counts
  const tagCounts: Record<string, number> = { All: updates.length };
  for (const u of updates) tagCounts[u.tag] = (tagCounts[u.tag] || 0) + 1;

  // Build unique game list
  const games = ['All', ...Array.from(new Set(updates.map((u) => u.game_name).filter(Boolean) as string[])).sort()];
  const gameCounts: Record<string, number> = { All: updates.length };
  for (const u of updates) {
    if (u.game_name) gameCounts[u.game_name] = (gameCounts[u.game_name] || 0) + 1;
  }

  const TAG_KEYS = ['All', 'Update', 'New Script', 'Patch', 'Maintenance', 'Announcement'];

  const filtered = updates.filter((u) => {
    if (filterMode === 'type') return activeTag === 'All' || u.tag === activeTag;
    return activeGame === 'All' || u.game_name === activeGame;
  });
  ...
  return (
    <div>
      {/* Mode toggle */}
      {hasGames && (
        <div className="flex items-center gap-1.5 mb-6 p-1 rounded-full w-fit" style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
          {(['type', 'game'] as FilterMode[]).map((mode) => (
            <button
              key={mode}
          ...
          : games.map((game) => {
              ...
              return (
                <button
                  ...
                  style={isActive ? { backgroundColor: 'rgba(201,169,122,0.15)', color: '#c9a97a', borderColor: 'rgba(201,169,122,0.35)' } : {}}
                  className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold border transition-all duration-150 cursor-pointer ${isActive ? '' : 'border-white/10 text-white/40 hover:text-white/70 hover:border-white/20'}`}
                >
                  {game}
                  <span className="text-[11px] font-bold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: isActive ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.06)', color: isActive ? 'inherit' : 'rgba(255,255,255,0.3)' }}>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 rounded-2xl" style={{ border: '1px solid rgba(255,255,255,0.06)', backgroundColor: 'rgba(255,255,255,0.015)' }}>
          ...
          <p className="text-sm" style={{ color: 'rgba(255,255,255,0.25)' }}>Try a different filter.</p>
        </div>
      ) : (
                ...
                <div className="flex flex-col flex-1 p-5 gap-3">
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Game name badge (when no banner image) */}
