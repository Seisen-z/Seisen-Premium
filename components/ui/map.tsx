      ...
      >
        <style>{`
          ...
          .maplibregl-popup-tip {
            ...
            border-left-color: rgba(18, 18, 18, 0.95) !important;
            border-right-color: rgba(18, 18, 18, 0.95) !important;
          }
        `}</style>
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-[#121212]/95 text-white border border-white/10 relative max-w-64 rounded-xl p-3 shadow-2xl backdrop-blur-md font-sans",
...
function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "bg-[#121212]/95 text-white border border-white/15 pointer-events-none rounded-lg px-2.5 py-1 text-xs shadow-xl backdrop-blur-md whitespace-nowrap font-sans",
...
>({
  ...
  useEffect(() => {
    ...
    const unclusteredCountLayerId = `unclustered-count-${id}`;
    ...
    // Add count text label inside unclustered point circles
    map.addLayer({
    ...
    return () => {
      try {
        if (map.getLayer(unclusteredCountLayerId))
          map.removeLayer(unclusteredCountLayerId);
